...
class stock_analytics_send_foreign_3_days(AppConfig):
    ...
    def send_3_days(self):
        try:
            ...
            df['3_days_return'] = df.groupby('stock_name')['close_price'].transform(lambda x: (x / x.shift(3) - 1) * 100)
            df.dropna(subset=['3_days_return'], inplace=True)
            top_gainers_3_days = df.sort_values(by='3_days_return', ascending=False).head(30)
            top_tickers = top_gainers_3_days['stock_name'].unique()
            print(top_tickers)
            ...
            plt.show()
            functions.send_png_tg(plt, caption='Foreign Stock Analysis: 3 Days Maximum returns')
