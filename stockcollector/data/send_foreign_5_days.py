...
class stock_analytics_send_foreign_5_days(AppConfig):
    ...
    def send_5_days(self):
        try:
            ...
            df['5_days_return'] = df.groupby('stock_name')['close_price'].transform(lambda x: (x / x.shift(5) - 1) * 100)
            df.dropna(subset=['5_days_return'], inplace=True)
            top_gainers_5_days = df.sort_values(by='5_days_return', ascending=False).head(30)
            top_tickers = top_gainers_5_days['stock_name'].unique()
            print(top_tickers)
            ...
            plt.show()
            functions.send_png_tg(plt, caption='Foreign Stock Analysis: 5 Days Maximum returns')
