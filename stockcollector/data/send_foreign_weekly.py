...
class stock_analytics(AppConfig):
    ...
    def send_last_3_months(self):
        try:
            ...
            filtered_stocks['volume_change_5_days'] = filtered_stocks.groupby('stock_name')['volume'].transform(
                lambda x: (x.rolling(window=5).mean() / x.rolling(window=5).mean().shift(5) - 1) * 100
            )
            
            volume_increase_threshold = 250
            ...
            plt.show()
            functions.send_png_tg(plt, caption='Foreign Stock Analysis: 3-Month Decline with Recent Surge in Trading Volume')
